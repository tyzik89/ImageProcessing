...
public class ShowImage {
    ...
    public static void showScatterChart(int type, Mat data, String title, Mat ... params) {
        Stage stage = new Stage();
        stage.setTitle(title);
        stage.initModality(Modality.NONE);
        ScrollPane scrollPane = new ScrollPane();
        ...
        if (type == 0) {
            ...
            xAxis.setLabel("Яркость");
            yAxis.setLabel("Количество");
            sc.setTitle("Гистограмма распределения яркостей");
            ...
            for (int i = 0; i < data.cols(); i++) {
                double val = data.get(0, i)[0];
                if (val == 0) continue;
                series1.getData().add(new XYChart.Data(String.valueOf(i), val));
            }
            ...
            if (params != null) {
                for (Mat param : params) {
                    ...
                    for (int i = 0; i < param.cols(); i++) {
                        ...
                        if (val == 0) continue;
                        series0.getData().add(new XYChart.Data(String.valueOf(val), -10));
                    }
